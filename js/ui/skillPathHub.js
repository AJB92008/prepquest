...
// satMathHub.js's own ZONES) started a second family, "Rolling Curve" —
...
// flat on a diagram. Curve Shaper still uses it: a real parabola arc
// with its own marked vertex at every stop, alternating a hill (opens
// down) and a valley (opens up). Root Finder and Expression Rebuilder's
...
// tests/curveReachLessonThemes.test.js's own dedicated check). All
// three still keep their whole composition on one fixed side of `p` —
// always above it, never alternating toward the boss clearing one row
// below — which is what keeps every one of them boss-safe
// unconditionally, without needing the near-boss special-casing Slope
// Fields' own crossingLines.js/boundaryLine.js required; see tests/
// curveReachLessonThemes.test.js's own dedicated sweep for the swept
// numbers.
